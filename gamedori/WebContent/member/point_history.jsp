...
<%
  ...
  if(isSearch){//검색
     ...
     count = phdao.getCount(member_no,auth);
  }
  ...
  if(isSearch){
     ...
     list = phdao.getListForNormal(member_no,start ,finish);    
	  ...
	  if(isSearch){
		     ...
		     list = phdao.getListAll(start ,finish); 
  }
  }
%>
...
<div align="center">
  ...
  <% if(startBlock > 1){ %>
     ...
     <%if(!isSearch){ %>
        <a href="point_history.jsp?page=<%=startBlock-1%>">[이전]</a>
     <%}else{ %>
        <a href="point_history.jsp?page=<%=startBlock-1%>&type=<%=type%>&keyword=<%=keyword%>">[이전]</a>
     <%} %>
  ...
  <%for(int i=startBlock; i <= finishBlock; i++){ %>
     <%if(!isSearch){ %>
     <!-- 목록일 경우 페이지 번호만 전달 -->
     <a href="point_history.jsp?page=<%=i%>"><%=i%></a>
     <%}else{ %>
     <!-- 검색일 경우 페이지 번호와 검색 분류(type), 검색어(keyword)를 전달 -->
     <a href="point_history?page=<%=i%>&type=<%=type%>&keyword=<%=keyword%>"><%=i%></a>
     <%} %>
  <%} %>
 
  <%if(pageCount > finishBlock){ %>
     <%if(!isSearch){ %>
        <a href="point_history.jsp?page=<%=finishBlock + 1%>">[다음]</a>
     <%}else{ %>
        <a href="point_history.jsp?page=<%=finishBlock + 1%>&type=<%=type%>&keyword=<%=keyword%>">[다음]</a>
     <%} %>
  <%} %>
  </h4>
 
  <!-- 검색창 -->
  <form action="point_history.jsp" method="get">
     <!-- 검색분류 -->
     <select name="type">
