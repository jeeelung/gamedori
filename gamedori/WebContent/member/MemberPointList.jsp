...
​
     ...
     <th>기존 유형</th>
     <td>
        <select name="point_no" onchange ="setPoint2();" id="point_list2">
              <option value="">선택</option>
           <%for(Map<String,Object> pdto : list){ %>
              <option data-score="<%=pdto.get("point_score")%>" data-type="<%=pdto.get("point_type")%>"value="<%=pdto.get("point_no") %>"><%=pdto.get("point_type") %></option>
           <%} %>
     ...
     <th>변경할 유형</th>
     <td>
        <input type="text" name ="point_type" id="point_type2">
     </td>
     </tr>
     <tr>
     <th>점수</th>
     <td>
        <input type="text" name ="point_score" id="point_score2">
     </td>
     </tr>
     <tr>
     <th colspan="2" rowspan="2">
...
function setPoint2(){
	  const point_list = document.querySelector("#point_list2");
	  let option_selected = point_list.options[point_list.selectedIndex];
	 
	  let point_type = document.querySelector("#point_type2");
	  let point_score = document.querySelector("#point_score2");
