...
public class NoticeFileDao {
	...
	public void delete(int notice_no) throws Exception {
		Connection con = getConnection();
		String sql = "DELETE FROM notice_file WHERE notice_no = ?";
		PreparedStatement ps = con.prepareStatement(sql);
	...
	// 게시물에 해당하는 파일 번호 전부 가져오기
		public List<Integer> getfileNo(int notice_no) throws Exception {
			Connection con = getConnection();
			String sql = "SELECT file_no FROM notice_file WHERE notice_no = ?";
			PreparedStatement ps = con.prepareStatement(sql);
			ps.setInt(1, notice_no);
